...
void MenuItemScale::onAfterDraw(){
  drawNum(value, 0);
}

void MenuItemScale::setValue(int value)
{
...
void MenuItemScale::onEnter() {
  ...
  do {
    ...
    M5.update();
    if (M5.BtnA.wasPressed() || M5.BtnA.pressedFor(msecHold)) { ++repeat; setValue(value==0?maximum:value - 1); }
    if (   btn1.wasPressed() ||    btn1.pressedFor(msecHold)) { ++repeat; setValue(value==maximum?0:value + 1); }
    btnDrawer.draw();
    if (pv != value) {
      drawNum(value, 1);
      pv = value;
...
{
  applyFont();
  String str(Scale::getAvailableScales()[value]->name());
